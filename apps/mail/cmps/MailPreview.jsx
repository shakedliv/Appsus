...
export function MailPreview({ mail, removeMail, toggleIsRead }) {
    ...
    return (
        <Link to={`/mail/${mail.id}`}>
            <article
                className={`mail-prev flex ${mailRead ? 'is-read' : ''}`}
            ...
            >
                ...
                <p className='mail-date'>
                    {' '}
                    <span>{utilService.getMonthName(sentAt)}</span>{' '}
                    <span>{utilService.getDayInMonth(sentAt)}</span>
                </p>
                <p className='mail-operations'>
                    <span className='remove-mail-btn' onClick={onRemoveMail}>
                        🗑️
                    </span>
                    <span className='toggle-read-btn' onClick={onToggleIsRead}>
                        📩
                    </span>{' '}
                </p>
            </article>
